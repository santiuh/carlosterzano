<template>
    <nav class="hidden xl:flex gap-4 text-white justify-between flex-col-reverse xl:flex-row fixed xl:static w-full transition-all duration-500 z-50">
    ...
    <nav class="flex flex-col w-full bg-white xl:hidden fixed z-50 ">
      <div class="w-full flex flex-row justify-between px-4 py-3">
        <div class="w-[24px]"></div>
        <NuxtLink class="items-center flex" to="/"><NuxtPicture :class="menu? 'opacity-0': 'opacity-100' " class="self-center transition-all duration-300" src="/logo_negro.svg"></NuxtPicture></NuxtLink>
        <NuxtPicture @click="menu = !menu" class="pt-1"  src="/menu.svg"></NuxtPicture>
     </div>
      <div :class="menu ? 'h-44' : 'h-0'" class="flex  flex-col font-semibold text-xl gap-4 pl-8  overflow-hidden transition-all">
          <NuxtLink @click="menu = false" class="hover:text-primary hover:cursor-pointer transition-all duration-300" to="/"> Inicio</NuxtLink>
          <NuxtLink @click="menu = false" class="hover:text-primary hover:cursor-pointer transition-all duration-300" :to="'/' + dato" v-for="dato in data">{{ dato }}</NuxtLink>
      </div>
    </nav>
  </template>
